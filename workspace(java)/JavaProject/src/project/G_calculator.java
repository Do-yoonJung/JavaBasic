...
public class G_calculator extends JFrame {
	...
	public G_calculator() {
		...
		JLabel lblNewLabel_1_1_1_1 = new JLabel(Integer.toString(pay)+"원");
		lblNewLabel_1_1_1_1.setFont(new Font("한컴 고딕", Font.PLAIN, 15));
		lblNewLabel_1_1_1_1.setHorizontalAlignment(SwingConstants.LEFT);
		lblNewLabel_1_1_1_1.setBounds(143, 137, 212, 15);
		contentPane.add(lblNewLabel_1_1_1_1);
		
		JLabel lblNewLabel_1_1_2 = new JLabel(Integer.toString(pTime)+"분");
	...
	void select() {
		try {
			...
			while(rs.next()) {
				...
				math = Math.round((float)pTime/10);
				pay =  (int)math* 500;
				if(pay>=50000) {
					pday = (int)((out_car.getTime()-in_car.getTime())/(24*60*60*1000));
					if(pday>=1) {
						pay = 0;
						math = Math.round(((float)pTime-(1440*(float)pday))/10);
						pay = ((int)math*500)+pday*50000;
						if((((int)math*500))>=50000){
							pay = (pday+1)*50000;
						}
					}
				}

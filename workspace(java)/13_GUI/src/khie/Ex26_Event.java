 ...
 * - 각 컴포넌트에 대하여 특정 행위를 하였을 때 그 행위에 대한 작업을 처리할 수 있도록 하는 것을 말함.
 *   사용자 또는 프로그램에 의해 발생할 수 있는 하나의 사건.
 * - 이벤트를 처리하는 자바 프로그램 코드로써 클래스를 만듦.
 *   이벤트 리스너틑 인터페이스를 제공하며, 개발자는 이 인터페이스를 상속을 받고 
 *   추상메서드를 구현하여 이벤트 리스너를 작성한다.
 ...
 *   2) 이벤트와 리스너 클래스 작성: 리스너 인터페이스를 상속받은 클래스를 작성하고 추상메서드를 모두 구현.
 *   3) 이벤트 리스너 등록: 이벤트를 받을 컴포넌트에 이벤트 리스너 등록
 * 
 * [이벤트 관련 용어]
 * - 이벤트 소스: 이벤트를 발생시킨 GUI컴포넌트를 말함.
 * - 이벤트 객체: 발생한 이벤트에 대한 정보(이벤트 종류, 이벤트 소스, 화면 좌표, 마우스 버튼의 종류, 눌려진 키)
 *             를 담는 객체로써, 이벤트에 따라 서로 다른 정보가 저장이 됨.

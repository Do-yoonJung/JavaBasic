 ...
 *    2) run() 메서드의 재정의. - 스레드 구현
 *    3-1) 스레드 클래스 객체 생성
 *    3-2) 참조변수.start() 메서드 호출 ==> run() 메서드 실행.
 */

class Thread1 extends Thread {  // 1-1)Thread 라는 클래스를 상속(extends)을 받음.
	
	@Override
	public void run() {  // 1-2) run() 메서드의 재정의. - 스레드 구현
		int i =1;
		while(true) {
...
class Thread2 extends Thread {  // 1-1)Thread 라는 클래스를 상속(extends)을 받음.

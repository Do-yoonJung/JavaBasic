 ...
 * 4. 자주 반복되는 내용의 정의해놓고, 필요할 때 호출하여 사용함.
 * 
 * 		형식)
 ...
 * 				(클래스, 메서드, 변수 앞에 사용됨)
 * 		- 반환형: 메서드를 실행하고 특정 타입의 결과를 반환해주겠다고 선언하는 것을 반환형(리턴타입) 이라고 말함.
 ...
 * 				반환형이 void인 경우를 제외하고 결과를 되돌려주어야 할 때는 메서드 명령문 맨 마지막에 
 * 				- return이라는 키워드를 작성 후, 앞에서 선언한 반환형과 같은 자료형으로 결과를 되돌려준다.
 * 		- 메서드이름: 식별자, 소문자로 시작, 두 개의 단어가 결합이 된 경우에는 카멜케이스로 작성
